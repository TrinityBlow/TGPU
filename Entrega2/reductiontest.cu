...
__global__ void vecMult2(double *d_matA,unsigned long n){      
    ...
    for( i = 1; i <= 2;  i++) {
        if( threadIdx.x < (int)(64 >> i)){
            s_mat[threadIdx.x] += s_mat[(threadIdx.x + (64 >> i))];
        }
        __syncthreads();
    }
...
int main(int argc, char *argv[]){
    ...
    unsigned long CUDA_BLK = 64,GRID_BLK;
    ...
    for(i = 4 ; i <= N; i *= 4){
        ...
        vecMult2<<<dimGrid, dimBlock>>>(d_matrices,(4*4*N) / (i / 2));
        cudaThreadSynchronize();
    }
